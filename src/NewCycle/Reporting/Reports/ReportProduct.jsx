...
export default function ReportProduct() {
    ...
    const getData = async () => {
        setLoading(true)
        let res = await axios.get("http://192.168.26.15/itemcreation-test/api/products");
    ...
    useEffect(() => {
        if (rowsOfData.length > 0) {
            let btn = document.getElementsByClassName('MuiButtonBase-root MuiIconButton-root MuiIconButton-sizeSmall css-1pe4mpk-MuiButtonBase-root-MuiIconButton-root')[16];
            console.log(btn);
            btn.click()
        }
    }, [rowsOfData])
